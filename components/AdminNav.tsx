...
export function AdminNav() {
  ...
  const isBookings = path === '/admin' || (path.startsWith('/admin') && !path.startsWith('/admin/customers') && !path.startsWith('/admin/jobs') && !path.startsWith('/admin/revenue'))
  const isCustomers = path.startsWith('/admin/customers')
  const isJobs = path.startsWith('/admin/jobs')
  const isRevenue = path.startsWith('/admin/revenue')
  ...
  return (
    <div className="flex gap-2 mb-5 flex-wrap">
      ...
      <Link
        href="/admin/revenue"
        className={`px-5 py-2 rounded-xl text-sm font-semibold transition-colors border
          ${isRevenue ? 'bg-gray-900 text-white border-gray-900' : 'bg-white text-gray-500 border-gray-200 hover:text-gray-900'}`}
      >
        Revenue
      </Link>
    </div>
